...
import { Condition, AgentType } from '../types';
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 p-6 text-center">
      ...
      {DEBUG && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl">
          ...
          >
            <h2 className="text-lg font-semibold text-white mb-2">Debug: Force Eliza</h2>
            <p className="text-gray-400 text-sm">
              Condition: Eliza vs. Gemini (forced Eliza)
            </p>
          </button>
          <button
            onClick={() => onSelectCondition(Condition.ELIZA_VS_GEMINI, AgentType.GEMINI_ELIZA)}
            className="group relative flex flex-col items-center p-6 bg-gray-800 border-2 border-red-700 rounded-xl hover:border-red-500 hover:bg-gray-750 transition-all duration-300"
          >
            <h2 className="text-lg font-semibold text-white mb-2">Debug: Force Gemini-Eliza</h2>
            <p className="text-gray-400 text-sm">
              Condition: Eliza vs. Gemini (forced Gemini)
            </p>
          </button>
          <button
            onClick={() => onSelectCondition(Condition.GEMINI_VS_STANFORD, AgentType.GEMINI_STUDENT)}
            className="group relative flex flex-col items-center p-6 bg-gray-800 border-2 border-red-700 rounded-xl hover:border-red-500 hover:bg-gray-750 transition-all duration-300"
          >
            <h2 className="text-lg font-semibold text-white mb-2">Debug: Force Gemini Student</h2>
            <p className="text-gray-400 text-sm">
              Condition: Gemini vs. Stanford (forced Gemini)
            </p>
          </button>
          <button
            onClick={() => onSelectCondition(Condition.GEMINI_VS_STANFORD, AgentType.REAL_STUDENT)}
            className="group relative flex flex-col items-center p-6 bg-gray-800 border-2 border-red-700 rounded-xl hover:border-red-500 hover:bg-gray-750 transition-all duration-300"
          >
