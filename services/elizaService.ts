/**
 * Ported from elizabot.js v1.1 (original)
 * https://github.com/oren/eliza-bot
 */

interface Decomp {
  0: string; // pattern
  1: string[]; // reassemblies
  2?: boolean; // memFlag
}
...
interface Keyword {
  ...
  1: number; // rank
  2: Decomp[]; // decomps
  3?: number; // original index (internal sorting)
}
...
export class ElizaBot {
  ...
  elizaInitials: string[];
  elizaKeywords: Keyword[];
  elizaPostTransforms: (string | RegExp)[];
  elizaFinals: string[];
  elizaQuits: string[];
  elizaPres: string[];
  elizaPosts: string[];
  ...
  preExp: RegExp | null = null;
  postExp: RegExp | null = null;
  
  _dataParsed: boolean = false;
  ...
  _init() {
    ...
    if (this.elizaSynons && typeof this.elizaSynons == 'object') {
      for (let i in this.elizaSynons) synPatterns[i] = '(' + i + '|' + this.elizaSynons[i].join('|') + ')';
    }
  ...
  _sortKeywords(a: Keyword, b: Keyword) {
    // sort by rank
    if (a[1] > b[1]) return -1
    else if (a[1] < b[1]) return 1
    // or original index
    else if ((a[3] || 0) > (b[3] || 0)) return 1
    else if ((a[3] || 0) < (b[3] || 0)) return -1
    else return 0;
  }
  ...
  _execRule(k: number, sentence: string) {
    ...
    for (let i = 0; i < decomps.length; i++) {
      ...
      if (m != null) {
        ...
        if (rpl.search(/^goto /i) == 0) {
          ...
          if (ki >= 0) return this._execRule(ki, sentence);
        }
  ...
  _postTransform(s: string) {
    ...
    if (this.elizaPostTransforms && this.elizaPostTransforms.length) {
      for (let i = 0; i < this.elizaPostTransforms.length; i += 2) {
        s = s.replace(this.elizaPostTransforms[i], this.elizaPostTransforms[i + 1] as string);
        if (this.elizaPostTransforms[i] instanceof RegExp) {
             (this.elizaPostTransforms[i] as RegExp).lastIndex = 0;
        }
      }
    }
